...
const emit = defineEmits(['toggle-pwd', 'copy', 'edit', 'delete', 'toggle-tag', 'move', 'toggle-select'])
...
<template>
  <div
    :class="['entry-card', selectable && 'entry-card--selectable', selected && 'entry-card--selected']"
    @click="onCardClick"
  >
    <div class="entry-card__header">
      <div class="entry-card__info">
        <label v-if="selectable" class="entry-card__checkbox" @click.stop>
          <input
            type="checkbox"
            :checked="selected"
            @change="emit('toggle-select')"
          />
          <span class="entry-card__checkbox-mark" aria-hidden="true">
            <svg v-if="selected" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
          </span>
        </label>
    ...
    <div v-if="!selectable" class="entry-card__footer">
      <button type="button" class="entry-card__action" @click.stop="emit('edit')">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
        编辑
      </button>
      <button type="button" class="entry-card__action" @click.stop="emit('move')">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        移动
      </button>
      <button v-if="entry.note" type="button" :class="['entry-card__action', noteExpanded && 'entry-card__action--active']" @click.stop="noteExpanded = !noteExpanded">
      ...
      <button type="button" class="entry-card__action entry-card__action--danger" @click.stop="emit('delete')">
...
.entry-card--selectable { cursor: pointer; user-select: none; }
.entry-card--selected { border-color: Highlight; box-shadow: 0 0 0 2px color-mix(in srgb, Highlight 25%, transparent); background: color-mix(in srgb, Highlight 4%, Canvas); }
.entry-card--selected:hover { border-color: Highlight; box-shadow: 0 0 0 2px color-mix(in srgb, Highlight 35%, transparent); }

.entry-card__checkbox { display: inline-flex; align-items: center; justify-content: center; flex-shrink: 0; cursor: pointer; }
.entry-card__checkbox input { position: absolute; opacity: 0; width: 0; height: 0; pointer-events: none; }
.entry-card__checkbox-mark { display: inline-flex; align-items: center; justify-content: center; width: 18px; height: 18px; border-radius: 5px; border: 1.5px solid color-mix(in srgb, CanvasText 25%, transparent); background: color-mix(in srgb, Canvas 80%, transparent); color: #fff; transition: all 0.15s ease; }
.entry-card--selected .entry-card__checkbox-mark { background: Highlight; border-color: Highlight; }
...
@media (prefers-color-scheme: dark) {
  ...
  .entry-card--selected { background: color-mix(in srgb, Highlight 10%, #1f1f23); }
}
